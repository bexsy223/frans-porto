...
(function ($) {
    ...
    $(function () {
        ...
        openMbMenu();
        clickActive();
        $("form.form-cta").on("submit", function (e) {
            e.preventDefault();
            var $form = $(this);
            var $status = $form.find(".form-status");
            $status.removeClass("text-primary").text("");
            $.ajax({
                ...
                $status.addClass("text-primary").text("Terima kasih! Pesan kamu sudah terkirim.");
                $form[0].reset();
                $(".select_js").each(function () {
                    var $sel = $(this);
                    $sel.val("");
                    $sel.next(".nice-select").find(".current").text($sel.find("option:selected").text());
                });
            }).fail(function () {
                $status.text("Maaf, terjadi masalah saat mengirim. Coba lagi.");
            });
        });
    });
})(jQuery);
