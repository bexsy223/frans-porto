...
window.onload = function () {
    ...
    if (document.querySelector(".section-selected-work")) {
        ...
        // Autoplay/pause video sesuai slide aktif
        function playActiveSlideVideo(index) {
            // Pause semua video
            $(".slick-for .slick-slide video").each(function () {
                try {
                    this.pause();
                } catch (e) {}
            });
            // Play video pada slide aktif
            const $activeVideos = $(".slick-for .slick-slide").eq(index).find("video");
        ...
        // Setiap perubahan slide
        $for.on("afterChange", function (event, slick, currentSlide) {
            playActiveSlideVideo(currentSlide);
        });
    }
};
